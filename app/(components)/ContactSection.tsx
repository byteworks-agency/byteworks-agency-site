...
export default function ContactSection() {
  ...
  const [values, setValues] = useState<FormValues>({ name: '', email: '', projectType: 'Website', message: '', lang });
  ...
  const canSubmit = useMemo(() => {
    ...
    if (!z.string().email().safeParse(values.email).success) return false;
    return true;
  }, [sending, values]);
  ...
  const handleBlur = (key: keyof FormValues) => () => validateField(key, values[key]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOk(true);
      setValues({ name: '', email: '', projectType: 'Website', message: '', lang });
      setFieldErrors({});
    } catch (err: any) {
      setOk(false);
        ...
        <form className="mt-8 max-w-xl grid grid-cols-1 gap-4" onSubmit={onSubmit} noValidate>
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-md border px-3 py-2 bg-white" placeholder="John Doe"
              autoComplete="name" aria-invalid={!!fieldErrors.name} aria-describedby={fieldErrors.name ? 'name-error' : undefined}
            />
            {fieldErrors.name && <p id="name-error" className="mt-1 text-sm text-rose-600">{fieldErrors.name}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-md border px-3 py-2 bg-white" placeholder="john@email.com"
              autoComplete="email" aria-invalid={!!fieldErrors.email} aria-describedby={fieldErrors.email ? 'email-error' : undefined}
            />
            {fieldErrors.email && <p id="email-error" className="mt-1 text-sm text-rose-600">{fieldErrors.email}</p>}
          </div>

          <div>
            <label htmlFor="projectType" className="text-sm">{dict.contact.project_type}</label>
            <select
              id="projectType" name="projectType" value={values.projectType} onChange={handleChange('projectType')}
          ...
          <div>
            ...
            <textarea
              ...
              value={values.message} onChange={handleChange('message')} onBlur={handleBlur('message')}
              className="mt-1 w-full rounded-md border px-3 py-2 bg-white" placeholder={dict.contact.placeholder}
              aria-invalid={!!fieldErrors.message} aria-describedby={fieldErrors.message ? 'message-error' : undefined}
            />
            {fieldErrors.message && <p id="message-error" className="mt-1 text-sm text-rose-600">{fieldErrors.message}</p>}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-2">
            ...
            <button type="button" onClick={sendWhatsApp} className="btn w-full sm:w-auto inline-flex items-center gap-2">
              <span className="icon icon-sm" style={{ ['--icon' as any]: "url('/icons/whatsapp.svg')" }} aria-hidden />
          ...
          <div className="min-h-[1.25rem]" aria-live="polite">
            ...
            {ok === false && <p className="text-sm text-rose-600 mt-2">{T.error}: {errorMsg}</p>}
          </div>
        </form>
      </div>
    </section>
